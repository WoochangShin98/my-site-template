...
export const defaultConfig = {
    ...
    items: [{"name":"Home","url":"#hero","icon":"Home","show":true},{"name":"Experience","url":"#about","icon":"User","show":true},{"name":"Projects","url":"#projects","icon":"Briefcase","show":true},{"name":"Contact","url":"#contact","icon":"Mail","show":true},{"name":"갤러리","url":"#gallery","icon":"Camera","show":false},{"name":"블로그","url":"#blog","icon":"Book","show":false}],
    siteTitle: "Woochang Shin Site",
    siteDescription: "A concise showcase of my projects, skills, and experience."
  }
                ...
                {navConfig.items.slice(0, 4).map((item, index) => (
                  <div key={index} className="flex items-center gap-2">
                    <label className="text-sm text-muted-foreground w-20">
                      {index === 0 ? '홈' : index === 1 ? 'Experience' : index === 2 ? 'Projects' : 'Contact'}
                    </label>
                    <input
                      type="text"
                      value={item.name}
